...
<div class="modal fade" id="disable_user_{{ $id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-content">
      ...
      <h6 class="modal-title" id="exampleModalLabel">Are you sure you want to change the user status?</h6>
      <button type="button" class="close" data-dismiss="modal" style="color: white"aria-label="Close">
        <span aria-hidden="true">&times;</span>
      </button>
    </div>
      <form method="POST" action="/pfc_chairmans/{{ $id }}/{{ $pfc_chairman->disabled ? 'enable' : 'disable' }}" style="display: inline-block;">
        @csrf
        {{ method_field('PATCH') }}
        <div class="card-body">
          @if (session('status'))
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-{{ $pfc_chairman->disabled ? 'success' : 'danger' }}">
