...
<div class="modal fade" id="disable_user_{{ $id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-header" style="background-color: #2580ff; color: white;">
      <h4 class="modal-title" id="exampleModalLabel">Are you sure you want to change the user status?</h4>
      <button type="button" class="close" data-dismiss="modal" style="color: white"aria-label="Close">
        <span aria-hidden="true">&times;</span>
      </button>
    </div>
      <form method="POST" action="/commission-heads/{{ $id }}/{{ $commission_head->disabled ? 'enable' : 'disable' }}" style="display: inline-block;">
        @csrf
        {{ method_field('PATCH') }}
        <div class="card-body">
          @if (session('status'))
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-{{ $commission_head->disabled ? 'success' : 'danger' }}">
